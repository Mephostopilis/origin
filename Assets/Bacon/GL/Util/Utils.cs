...
namespace Bacon.GL.Util {
    public class Utils {
        ...
        public static bool Raycast(Ray ray, float distance, Plane plane, out RaycastHit hitInfo) {
            hitInfo = new RaycastHit();
            float dn = Vector3.Dot(ray.direction, plane.normal);
            if (-1E-7 < dn && dn < 1E-7)
                return false; // parallel  

            Debug.Log("dIST: " + plane.GetDistanceToPoint(ray.origin));
            float dist = -plane.GetDistanceToPoint(ray.origin) / dn;
            if (dist < distance) {
